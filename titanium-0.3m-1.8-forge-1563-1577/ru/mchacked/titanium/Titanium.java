...
public class Titanium implements IKeyBind {
	...
	private void Method51() {
		if(Hooks.setts("ka")) {
			float kaRadius = 4.0F;
			this.Field46.clear();
			this.Field46.addAll(this.mc.theWorld.getEntitiesWithinAABBExcludingEntity(this.mc.thePlayer, this.mc.thePlayer.getEntityBoundingBox().expand((double)kaRadius, (double)kaRadius, (double)kaRadius)));
			...
			while(i$.hasNext()) {
				...
				if(!e.isDead) {
					...
					if(Hooks.setts("kap") && e instanceof EntityPlayer && !this.mc.thePlayer.getName().equals(e.getName()) && !this.kaFriends.Method26(e.getName())) {
						attack = true;
					}
		...
		if(Hooks.setts("eradar")) {
			...
			if(Hooks.setts("enear")) {
				...
				while(i$.hasNext()) {
					...
					if(!mc.thePlayer.getName().equalsIgnoreCase(ep.getName())) {
						double dist = (double)mc.thePlayer.getDistanceToEntity(ep);
						String st = String.format("§6%s §%c[%.1f]", new Object[]{ep.getName(), Character.valueOf((char)(dist < 16.0D?(dist < 5.0D?'c':'e'):'a')), Double.valueOf(dist)});
	...
	private void Method53() {
		if(this.Field48 || this.Field50) {
			...
			boolean space = focus && mc.gameSettings.keyBindJump.isKeyDown();
			boolean shift = focus && mc.gameSettings.keyBindSneak.isKeyDown();
			boolean keyW = focus && mc.gameSettings.keyBindForward.isKeyDown();
			boolean keyA = focus && mc.gameSettings.keyBindLeft.isKeyDown();
			boolean keyS = focus && mc.gameSettings.keyBindBack.isKeyDown();
			boolean keyD = focus && mc.gameSettings.keyBindRight.isKeyDown();
			if(space) {
	...
	private void Method55(String key, boolean value) {
		...
		if(key.equalsIgnoreCase("mn")) {
			...
			this.Field50 = value;
			if(value) {
				EntityPlayerSP eo = mc.thePlayer;
				this.Field51 = eo.posX;
				this.Field52 = eo.posY;
				this.Field53 = eo.posZ;
				this.Field54 = eo.rotationPitch;
				this.Field55 = eo.rotationYaw;
				EntityOtherPlayerMP eo1 = new EntityOtherPlayerMP(mc.theWorld, eo.getGameProfile());
				eo1.setPositionAndRotation(this.Field51, this.Field52, this.Field53, this.Field55, this.Field54);
	...
	public void Method58() {
		...
		for(int i$ = 0; i$ < len$; ++i$) {
			...
			a.saveConfig(ac);
			addons.setTag(a.getAddonID(), ac);
		}
		
		root.setTag("Addons", addons);
