...
public class EspUtils {
	...
	private boolean Method88(TileEntityChest tec) {
		Block block = tec.getWorld().getBlockState(tec.getPos()).getBlock();
	...
	private void Method89(TileEntityChest tec) {
		if(tec.adjacentChestXPos == null && tec.adjacentChestZPos == null) {
			int x = tec.getPos().getX();
			int y = tec.getPos().getY();
			int z = tec.getPos().getZ();
	...
	private void Method91(TileEntityMobSpawner tem) {
		int x = tem.getPos().getX();
		int y = tem.getPos().getY();
		int z = tem.getPos().getZ();
		...
		RenderGlobal.drawSelectionBoundingBox(aabb);
		if(d < 32.0D && d > 4.0D) {
			MobSpawnerBaseLogic msbl = tem.getSpawnerBaseLogic();
			...
			RenderGlobal.drawSelectionBoundingBox(aabb.expand(-0.20000000298023224D, -0.20000000298023224D, -0.20000000298023224D));
		}
	}
	...
	private void Method92(EntityMinecart emc) {
		...
		RenderGlobal.drawSelectionBoundingBox(this.Method86(x - r, y - r, z - r, x + r, y + r, z + r));
	}
	...
	private void Method94(float f) {
		...
		while(i$.hasNext()) {
			...
			if(e instanceof EntityPlayer) {
				...
				if(!ep.getName().equalsIgnoreCase(mc.getSession().getUsername())) {
					double d = this.Method81(ep.posX, ep.posY, ep.posZ);
					float a = this.Method84(d);
					GL11.glColor4f(1.0F, 0.3F, 0.3F, a);
					GL11.glLineWidth(this.Method83(d));
					RenderGlobal.drawSelectionBoundingBox(ep.getEntityBoundingBox());
				}
			}
		}
	}
