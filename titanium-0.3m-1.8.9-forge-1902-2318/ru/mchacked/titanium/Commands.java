...
public class Commands implements ICommandHandler {
	...
	private void echo(String s) {
		mc.thePlayer.addChatMessage(new ChatComponentTranslation(s));
	}
				...
				if(cmd.equals("xr")) {
					...
					if(cmd.equals("rocket")) {
						...
						if(args.length < 2) {
							...
							int pY = (int)mc.thePlayer.getEntityBoundingBox().minY;
							int pZ = (int)mc.thePlayer.posZ;
							for(int dX = -var27; dX <= var27; ++dX) {
								for(int dY = -var27; dY <= var27; ++dY) {
									for(int dZ = -var27; dZ <= var27; ++dZ) {
										Block block = mc.theWorld.getBlockState(new BlockPos(pX + dX, pY + dY, pZ + dZ)).getBlock();
