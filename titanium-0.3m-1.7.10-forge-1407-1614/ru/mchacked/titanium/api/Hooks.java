...
public class Hooks {
	...
	public static AxisAlignedBB jesus(BlockLiquid bf, int x, int y, int z) {
		return !setts("je") ? null : AxisAlignedBB.getBoundingBox((double)x + bf.getBlockBoundsMinX(), (double)y + bf.getBlockBoundsMinY(), (double)z + bf.getBlockBoundsMinZ(), (double)x + bf.getBlockBoundsMaxY(), (double)y + bf.getBlockBoundsMaxY(), (double)z + bf.getBlockBoundsMaxZ());
	}
	...
	public static void renderESP(RendererLivingEntity rendererLivingEntity, Entity e, double par2, double par4, double par6) {
		if(e instanceof EntityPlayer) {
			...
			if(sb.length() != 0) {
				rendererLivingEntity.func_96449_a(ep, par2, par4 + 0.5D, par6, sb.toString(), 1F, 1D);
			}
		}
	}
