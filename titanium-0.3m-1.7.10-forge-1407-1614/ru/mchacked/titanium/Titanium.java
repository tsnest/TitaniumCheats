...
public class Titanium implements IKeyBind {
		...
		if(Hooks.setts("ka")) {
			...
			while(i$.hasNext()) {
				...
				if(!e.isDead) {
					...
					if(Hooks.setts("kap") && e instanceof EntityPlayer && !this.mc.thePlayer.getCommandSenderName().equals(e.getCommandSenderName()) && !this.kaFriends.Method26(e.getCommandSenderName())) {
						attack = true;
					}
		...
		if(Hooks.setts("eradar")) {
			...
			if(Hooks.setts("enear")) {
				...
				while(i$.hasNext()) {
					...
					if(!mc.thePlayer.getCommandSenderName().equalsIgnoreCase(ep.getCommandSenderName())) {
						double dist = (double)mc.thePlayer.getDistanceToEntity(ep);
						String st = String.format("§6%s §%c[%.1f]", new Object[]{ep.getCommandSenderName(), Character.valueOf((char)(dist < 16.0D?(dist < 5.0D?'c':'e'):'a')), Double.valueOf(dist)});
	...
	private void Method53() {
		if(this.Field48 || this.Field50) {
			...
			boolean space = focus && mc.gameSettings.keyBindJump.getIsKeyPressed();
			boolean shift = focus && mc.gameSettings.keyBindSneak.getIsKeyPressed();
			boolean keyW = focus && mc.gameSettings.keyBindForward.getIsKeyPressed();
			boolean keyA = focus && mc.gameSettings.keyBindLeft.getIsKeyPressed();
			boolean keyS = focus && mc.gameSettings.keyBindBack.getIsKeyPressed();
			boolean keyD = focus && mc.gameSettings.keyBindRight.getIsKeyPressed();
			if(space) {
