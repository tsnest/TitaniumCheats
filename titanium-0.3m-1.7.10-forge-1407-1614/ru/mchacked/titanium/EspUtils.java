...
public class EspUtils {
	...
	private boolean Method88(TileEntityChest tec) {
		Block block = tec.getWorldObj().getBlock(tec.xCoord, tec.yCoord, tec.zCoord);
		int id = block.getIdFromBlock(block);
		if(id >= 0 && id <= 4095) {
			Block b = Block.getBlockById(id);
			return b != null && b instanceof BlockChest?((BlockChest)b).field_149956_a != 0:false;
		} else {
			return false;
		}
	}
	...
	private void Method91(TileEntityMobSpawner tem) {
		...
		RenderUtils.Method147(aabb);
		if(d < 32.0D && d > 4.0D) {
			MobSpawnerBaseLogic msbl = tem.func_145881_a();
	...
	private void Method94(float f) {
		...
		while(i$.hasNext()) {
			...
			if(e instanceof EntityPlayer) {
				...
				if(!ep.getCommandSenderName().equalsIgnoreCase(mc.getSession().getUsername())) {
