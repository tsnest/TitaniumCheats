...
public class Hooks {
	...
	public static AxisAlignedBB jesus(BlockFluid bf, int x, int y, int z) {
		return !setts("je") ? null : AxisAlignedBB.getAABBPool().addOrModifyAABBInPool((double)x + bf.getBlockBoundsMinX(), (double)y + bf.getBlockBoundsMinY(), (double)z + bf.getBlockBoundsMinZ(), (double)x + bf.getBlockBoundsMaxY(), (double)y + bf.getBlockBoundsMaxY(), (double)z + bf.getBlockBoundsMaxZ());
	}
	...
	public static void renderESP(RenderLiving rendererLivingEntity, Entity e, double par2, double par4, double par6) {
	...
	public static void autoFish(Packet28EntityVelocity ev) {
		if(setts("af")) {
			if(mc.thePlayer.getCurrentEquippedItem() != null) {
				if(mc.thePlayer.getCurrentEquippedItem().getItem() instanceof ItemFishingRod) {
					if(ev.motionX == 0 && ev.motionZ == 0 && ev.motionY < 0) {
						Entity e = mc.theWorld.getEntityByID(ev.entityId);
						if(e instanceof EntityFishHook) {
							mc.thePlayer.sendQueue.addToSendQueue(new Packet15Place(-1, -1, -1, 255, (ItemStack)null, 0.0F, 0.0F, 0.0F));
							mc.thePlayer.sendQueue.addToSendQueue(new Packet15Place(-1, -1, -1, 255, (ItemStack)null, 0.0F, 0.0F, 0.0F));
						}
					}
				}
